...
class wpDiscuzForm implements wpdFormConst
{
    ...
    public function customFormAdminScripts()
    {
        ...
        if ($current_screen->id === self::WPDISCUZ_FORMS_CONTENT_TYPE) {
            wp_register_style("fontawesome-iconpicker-css", rct_plugins_url(WPDISCUZ_DIR_NAME . "/assets/third-party/fontawesome-iconpicker/css/fontawesome-iconpicker.min.css"), [], "1.12.1");
            wp_enqueue_style("fontawesome-iconpicker-css");
            wp_register_script("fontawesome-iconpicker-js", rct_plugins_url(WPDISCUZ_DIR_NAME . "/assets/third-party/fontawesome-iconpicker/js/fontawesome-iconpicker.js"), ["jquery"], "1.12.1", true);
            wp_enqueue_script("fontawesome-iconpicker-js");
            wp_register_style("wpdiscuz-custom-form-css", rct_plugins_url(WPDISCUZ_DIR_NAME . "/assets/css/wpdiscuz-custom-form.css"), [], $this->pluginVersion);
            wp_enqueue_style("wpdiscuz-custom-form-css");
            wp_register_script("wpdiscuz-custom-form", rct_plugins_url(WPDISCUZ_DIR_NAME . "/assets/js/wpdiscuz-custom-form.js"), ["jquery"], $this->pluginVersion, true);
            ...
            wp_register_script("wpdiscuz-form-menu-item", rct_plugins_url(WPDISCUZ_DIR_NAME . "/assets/js/wpdiscuz-admin-menu-item.js"), ["jquery"], $this->pluginVersion, true);
    ...
    private function initAdminPhrases()
    {
        $this->wpdFormAdminOptions = [
            ...
            "loaderImg" => rct_plugins_url(WPDISCUZ_DIR_NAME . "/assets/img/form-loading.gif"),
        ];
    }
    ...
    public function changeCommentStatus($new_status, $old_status, $comment)
    {
        ...
        if ($old_status === "approved" && $rating) {
            $this->updatePostRating($comment, -1);
        } elseif ($new_status === "approved" && $rating) {
            $this->updatePostRating($comment, 1);
        }
    }
