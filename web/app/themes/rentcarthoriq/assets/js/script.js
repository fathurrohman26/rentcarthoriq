(function ($) {
  ...
  if ($("a.sewa-btn")?.length) {
    $("a.sewa-btn").on("click", function (e) {
      const price = $(this).data("price");
      const title = $(this).data("title");
      const number = $(this).data("number");

      if (price && title && number) {
        e.preventDefault();
        const message = encodeURIComponent(
          `Halo ${title}, Saya ingin tahu informasi lebih lanjut tentang Sewa *${price}*`
        );
        location.href = `https://wa.me/${String(
          number
        ).trim()}?text=${message}`;
      }
    });
    ...
    $(".contact-form-validated").each(function () {
      ...
      self.validate({
        ...
        submitHandler: function (form) {
          ...
          const phoneInput = $(form).find('input[name="Phone"]').val();
          const service = $(form).find("select.selectmenu").val();
          const message = $(form).find('textarea[name="message"]').val();

          const waNumber = $(form).find('input[name="wa_target"').val();

          const waMessage = `Halo, saya ${name} ingin membuat appointment.
Nama: ${name}
Email: ${email}
Telepon: ${phoneInput}
Layanan: Sewa ${service}
Pesan: ${message}`;
